...
train_dic = dict(zip(train_labels['file_name'], train_labels['category_id'])) # id: keys, class_num: value
...
with zipfile.ZipFile(zip_path, 'r') as zip_file:
    for i in train_dic.keys():
        with zip_file.open('train/'+i) as img_file:
            # 画像のバイナリデータを読み込む
...
# クラス数の定義
num_classes = 10 # 10クラス

# One-hot encoding
Y_train = np.array(train_labels['category_id'])
...
# ファイルのエクスポート
sub.to_csv('./submit/mnist_keras_CNN_baseline.tsv', sep='\t', header=None, index=None)